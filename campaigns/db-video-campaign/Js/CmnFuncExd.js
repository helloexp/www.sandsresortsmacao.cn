...
(function () {
    ...
    //---------------------------------------
    Cmn.Func.IsOrientationchangeByResize = false; //是否用Reize事件监听转屏
    ...
    if (Cmn.Func.IsOrientationchangeByResize) {
        $(window).resize(function () { alert("windwos Resize"); HasOrrentationchange = true; });
    }
    ...
    var _MainViewPort = ""; //保存主要方向的ViewPort
    var _LastSetViewPort = ""; //最后一次设置的ViewPort
        ...
        if (adviseVerticalImgUrl != undefined && adviseVerticalImgUrl!="" && Cmn.Func.IsString(adviseVerticalImgUrl)) {
            ...
            if ((Cmn.Func.IsHorizontalScreen() && mainViewIsHorizontal == false) ||
                ...
                if ($("body .AdviseVerticalImg").length <= 0) {
                    ...
                    $(".AdviseVerticalImg,.AdviseVerticalImgBg").stop().fadeIn(100, function () {
                        if ($(".cg-DefaultImgHint").length > 0) { _ajustImgPos(); } //系统默认的提示需要调整图片位置
                    });
                }

                ////调整显示位置
                //$(".AdviseVerticalImgBg,.AdviseVerticalImg").height($(window).height() * ($("body").width() / $(window).width()));
                //$(".AdviseVerticalImgBg,.AdviseVerticalImg").width($("body").width());
                //$(".AdviseVerticalImgBg,.AdviseVerticalImg").css("top",$(window).scrollTop());
                ...
                Cmn.DebugLog("ProcessAdviseVerticalImg 隐藏横竖屏提示" +
                    $(".AdviseVerticalImg,.AdviseVerticalImgBg").length);

                $(".AdviseVerticalImg,.AdviseVerticalImgBg").stop().hide();
    ...
    var SetViewPort = function (viewPortContent) {
        ...
        if (viewPortContent != _LastSetViewPort) {
            $("[name='viewport']").attr("content", viewPortContent);
            _MainViewPort = viewPortContent;
            _LastSetViewPort = viewPortContent;
            //_HasSetViewPort = true;
        }
    }

    var SetTmpViewPort = function (viewPortContent) {
        /// <summary>设置临时的viewPort</summary>
    ...
    Cmn.Func.MobileAdaptive = function (mainContentWidth, mainContentHeight, adviseVerticalImgUrl, adaptiveMode,
        ...
        Cmn.DebugLog("自适应版本3.3.7");
        //cms 创建静态页面时 不做自适应
        if (window.Cms && Cmn.Func.GetParamFromUrl("CmsState") == "IsCreateHtml") {
            return;
        }
        ...
        //修复横竖屏尺寸(因为这个函数传入的宽高可能有问题)
        if (mainContentHeight && mainContentWidth) {
            if ((Cmn.Func.IsHorizontalScreen() && mainContentWidth < mainContentHeight) ||
               (Cmn.Func.IsHorizontalScreen() == false && mainContentWidth > mainContentHeight)) {
                var _tmpInt = mainContentWidth;
                mainContentWidth = mainContentHeight;
                mainContentHeight = _tmpInt;
            }
        }
        ...
        if ((Cmn.Func.IsHorizontalScreen() && mainViewIsHorizontal == true) ||
            (Cmn.Func.IsHorizontalScreen() == false && mainViewIsHorizontal == false)) { //判断是否是主方向
                        ...
                        if (Cmn.Func.IsWeiXin()) {
                            ...
                            if (mainViewIsHorizontal === true) { //横屏浏览网站 
                                ...
                                if (Cmn.Func.IsHorizontalScreen()) { _width = mainContentHeight; }
                                    ...
                                    //_width = mainContentWidth;
                                    //ford站 需要在其它浏览器正常浏览做的调整 at 20150513
                                    if (mainContentWidth * Cmn.Func.GetHeightWidthRate() < mainContentHeight) {
                                        _width = mainContentHeight / Cmn.Func.GetHeightWidthRate();
                                    }
                                }
                    ...
                    if (_HasTargetDensitydpi && (_androidVersion < 4.4 || navigator.userAgent.indexOf("4.4.4") >= 0)) { //这个属性设置过以后，再设置viewport就没有用了
                        ...
                        //else {
                            ...
                            if (_densitydpi <= 400) { //超过400，手机上无效
                                ...
                                var _lowerUserAgent = navigator.userAgent.toLowerCase();

                                //华为的两个机型感觉他机器自己算过自适应，所有如果设置放大倍数的话，画面或缩得很小
                                if (_androidVersion == 4.2 || (_lowerUserAgent.indexOf("huawei") >= 0 && _androidVersion>=4.4)) { //设置放大倍数没有用
            ...
            if (Cmn.Func.IsIOS()) {
                if (Cmn.Func.IsIphone5()) {
                    ...
                    if (Cmn.Func.IsWeiXin()) {
                        //为了解决，输入框输入东西后，横屏提示没有全屏的问题
                        if (Cmn.Func.IsHorizontalScreen() && _LastSetViewPort!="") { SetTmpViewPort("width=1136,user-scalable=no;"); }
                    }
                }
            }
    ...
    Cmn.Func.ChangeBrowserUrlTo = function (url) {
        ...
        if (url.indexOf("http:") < 0 && url.indexOf("https:") < 0) { //不是绝对路径
    ...
    Cmn.Func.ImagePreload = function (selector, progressCallback, completeCallback, realUrlAttrName, loadCount) {
        ...
        function _preload(src, obj, index) {
            ...
            obj.onload = function () {!_isComplete && _loadComplete(obj, src, false, index); };
            obj.onerror = function () { !_isComplete && _loadComplete(obj, src, true, index); }
            obj.src = src;
            _isComplete = obj.complete;
            if (_isComplete) { _loadComplete(obj, src, false, index); }
        }
        ...
        function _loadComplete(obj, src, isErr, index) {
            ...
            if (_loadCount >= _loadTotal) { completeCallback && completeCallback(_error); }
                ...
                if (_loadCount % loadCount == 0) { _load(_loadCount); }
            }
        }
    }
    //------------------------------------图片预加载
    Cmn.Func.ImageLazyLoading = function (selector, progressCallback, completeCallback, realUrlAttrName, loadCount) {
        /// <summary>图片预加载 该方法已过期 请用 Cmn.Func.ImagePreload</summary>
        /// <param name="selector" type="string"> 要懒加载图片容器选择器 </param>
        /// <param name="progressCallback" type="function"> progressCallback 进度总大小为100</param>
        /// <param name="callback" type="function">所有图片loading完的回调</param>
        /// <param name="realUrlAttrName" type="string">真实路径的属性名称 默认lazypath </param>
        /// <param name="loadCount" type="int">每批加载的图片个数 </param>
        this.ImagePreload(selector, progressCallback, completeCallback, realUrlAttrName, loadCount);
    }
    //------------------------------------图片懒加载
    Cmn.Func.ImageLazyLoad = function (selector,realUrlAttrName) {
        /// <summary>图片懒加载</summary>
        /// <param name="selector" type="string"> 要懒加载图片容器选择器 </param>
        /// <param name="realUrlAttrName" type="string">真实路径的属性名称 默认lazypath </param>
        ...
        var _container = $(selector);
          //图片对象
        var _images = _container.find("img[" + realUrlAttrName + "]").css({ "opacity": "0.1", "filter": "alpha(opacity=10)" });
        //容器距离屏幕顶端的距离
        var _containerTop = _container.offset().top;
        //加载的个数
        var _loadCount = -1;
        var _lastIndex = null;
        ...
        //scroll事件句柄
        var _scrollHeader = function () {
            //当前容器的scroll 高度


            if (_lastScrollTop < _container.scrollTop() || _lastScrollTop == 0) {

                _lastScrollTop = _container.scrollTop();

                _lazyLoad(_loadCount+1);
            }
           
        };


        function _lazyLoad(index) {

            var _image = _images.eq(index);
 
            if ((index > 0 && _images.eq(index - 1).offset().top - _containerTop > _container.height())
                || index > _images.length - 1
                || !_image.attr(realUrlAttrName)) {
               
            }
            else {
                var _img = new Image();
                _img.DOM = _image;
                _img.onload = function () { _loadComplete.call(this, _img); };
                _img.onerror = function () { _loadComplete.call(this, _img); };
                _img.src = _image.attr(realUrlAttrName);
                _image.removeAttr(realUrlAttrName);
                if (_img.complete) { _loadComplete.call(_img, _img); }
                _loadCount++;
            }
        ...
        //图片load的回调函数
        function _loadComplete(img) {
          
            this.DOM.attr("src", this.src);

            this.DOM.css({ "opacity": "1", "filter": "alpha(opacity=100)"});
            if (_images.eq(_loadCount).offset().top - _containerTop <= _container.height()
                   && _loadCount < _images.length - 1) {
                _lazyLoad(_loadCount+1);
            }
        }

        _container.off("scroll", _scrollHeader).on("scroll", _scrollHeader);

        _scrollHeader();
    }
    //------------------------------------长按保存图片
    Cmn.Func.HoldSaveImage = function (selector) {
        /// <summary>长按保存图片</summary>
        /// <param name="selector" type="String">要保存的图片选择器</param>
     

        $(selector).on("touchstart", function (e) {
            e.stopPropagation();

            //如果是qq内置浏览器 并且不是 爱疯手机的话 就另开一页 让其保存
            if (Cmn.Func.IsQQBrowser() && !Cmn.Func.IsIOS()) {
    ...
    Cmn.Func.FrameAnimation = (function () {
        ...
        var _FrameAnimation = function (selector, speed, count, callback) {
            this.frames = $(selector);
            this.frames.eq(0).css({ "visibility": "visible" }).siblings().css({ "visibility": "hidden" });
            this.index = 0;
            this.interval = 0;
        ...
        _FrameAnimation.prototype = {
            Run: function (speed, count, callback) {
                ...
                _sefl.interval = setInterval(function () {
                    _sefl.frames.eq(_sefl.index).css({ "visibility": "visible" }).siblings().css({ "visibility": "hidden" });
                    _sefl.index++;
                    if (_sefl.frames.length <= _sefl.index) {
                        count--;
            ...
            if ((Cmn.Func.IsHorizontalScreen() && mainViewIsHorizontal == false) ||
               ...
               // $("body").css("width", "100%");
                ...
                var _ajustImgPos = function () {
                    ...
                    //调整显示位置
                    $(".AdviseVerticalImgBg,.AdviseVerticalImg").height($(window).height() * ($("body").width() / $(window).width())+2);
                    $(".AdviseVerticalImgBg,.AdviseVerticalImg").width($("body").width());
                    $(".AdviseVerticalImgBg,.AdviseVerticalImg").css("top", ($(window).scrollTop() *
                        ($("body").width() / $(window).width())-1)+"px");
                ...
                if ($("body .AdviseVerticalImg").length <= 0) {
                    ...
                    //如果body大于窗口的话，有滚动条，需要监测事件
                    if ($("body").height() / ($("body").width() / $(window).width()) > $(window).height()) {
                    ...
                    $(".AdviseVerticalImg,.AdviseVerticalImgBg").show();
                    if ($(".cg-DefaultImgHint").length > 0) { _ajustImgPos(); } //系统默认的提示需要调整图片位置
                }
    ...
    Cmn.Func.WebsiteAdaptive = function (mainContentWidth, mainContentHeight, adviseVerticalImgUrl,adaptiveMode) {
        /// <summary>网站自适应类</summary>

        Cmn.DebugLog("Scale自适应版本1.1.3");

        //cms 创建静态页面时 不做自适应
        if (window.Cms && Cmn.Func.GetParamFromUrl("CmsState") == "IsCreateHtml") { return; }

        var _Self = this;
        var _bodyContainer = $(".cmn_BodyContainer"); //body容器

        if (_bodyContainer.length <= 0) { _bodyContainer = $("body"); }

        this.OnOrientationChange = new Cmn.Event(); //当方向改变时触发
        this.MainViewIsHorizontal = false; //主要视角是否是横屏
        this.VerticalSafeSize = { Width: mainContentWidth, height: mainContentHeight };
        this.HorizontalSafeSize = { Width:mainContentWidth, height: mainContentHeight };

        //设置通过通过Resize事件监听横竖屏切换
        Cmn.Func.IsOrientationchangeByResize = true;

        //默认为只适应宽度
        if (adaptiveMode == undefined) { adaptiveMode = Cmn.Func.MobileAdaptiveMode.Width; }

        var SetScale = function (scaleVal) {
            /// <summary>设置放大倍数</summary>

            _bodyContainer.css("-webkit-transform", "scale(" + scaleVal + ")");
            _bodyContainer.css("-webkit-transform-origin", "0% 0%");
            //$("body").css("left", $("body").width() * (scaleVal - 1)/2 + "px");
            //$("body").css("top", $("body").height() * (scaleVal - 1)/2 + "px");
        };

        this.Recalculate = function () {
            /// <summary>重新计算自适应</summary>

            Cmn.DebugLog("进入Recalculate,当前是:" + (Cmn.Func.IsHorizontalScreen()?"横屏":"竖屏"));
            ...
            //ProcessAdviseVerticalImg(adviseVerticalImgUrl, _Self.MainViewIsHorizontal);

            var _windowWidth = $(window).width();
            var _windowHeight = $(window).height();

            Cmn.DebugLog("$(window).width()的宽：" + $(window).width() + " window.innerWidth:" + window.innerWidth +
                " window.screen.width:" + window.screen.width + " window.screen.availWidth:" + window.screen.availWidth+
                " document.documentElement.clientWidth：" + document.documentElement.clientWidth);

            if (adaptiveMode == Cmn.Func.MobileAdaptiveMode.Width ||  //宽度自适应
                adaptiveMode == Cmn.Func.MobileAdaptiveMode.WidthCutOutHeight) {  //宽度自适应，高度超出裁切
                var _multiple = _windowWidth / mainContentWidth;

                // $("body,html").width(mainContentWidth);
                _bodyContainer.width(mainContentWidth);
               // $("body").height(_windowHeight / _multiple);

                SetScale(_multiple);

                Cmn.DebugLog("body的宽：" + $("body").width() + "  _multiple:" + _multiple);

                //如果body的高度不满屏幕高度的话，补满
                if ($("body").height() < _windowHeight / _multiple) {
                    _bodyContainer.height(_windowHeight / _multiple);
                }
                ...
                if (adaptiveMode == Cmn.Func.MobileAdaptiveMode.WidthCutOutHeight) {
                    ...
                    if (Cmn.Func.GetWindowsHeight() > mainContentHeight) {
                        ...
                        if ($("body").height() > Cmn.Func.GetWindowsHeight()) {
                            _bodyContainer.height(Cmn.Func.GetWindowsHeight());
                            _bodyContainer.css("overflow-y", "hidden");
                        }
                    }
                ...
                if (_windowHeight / mainContentHeight < _multiple) { _multiple = _windowHeight / mainContentHeight; }

                _bodyContainer.width(_windowWidth / _multiple);
                _bodyContainer.height(_windowHeight / _multiple);

                SetScale(_multiple);

                Cmn.DebugLog("屏幕宽:" + _windowWidth + "高:" + _windowHeight + "，放大倍数：" + _multiple +
                    " body宽:" + $("body").width() + " body高：" + $("body").height());
            }
        ...
        //旋转事件
        var _lastIsHorizontalScreen = Cmn.Func.IsHorizontalScreen(); //记录最后一次是不是横屏
        //var _last

        var _onOrientationchange = function (event) {
            _lastIsHorizontalScreen = Cmn.Func.IsHorizontalScreen();
        ...
        $(window).resize(function () {
            //如果是安卓，输入法导致的需要忽略掉
            if (Cmn.Func.IsAndroid() && Cmn.Func.IsHorizontalScreen()==false && $(window).width() > $(window).height()) {
                Cmn.DebugLog("可能是输入法导致尺寸变化，忽略"); return;
            }

            _onOrientationchange();

            setTimeout(_onOrientationchange,200);
        });

   
        //判断viewprot有没有加，没加的话提示，放在这里是为了只做一次
        if ($("[name='viewport']").length < 1) {
            alert("页面上必须要加上默认的viewport。(<meta content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no;' name='viewport' />)");
        }
...
(function ($,window) {
    ...
    $.extend($.fn, {
        ...
        Cg: function () {
            /// <summary>cagoe对于jQuery的方法扩展</summary>
            /// <returns type="_CgFormJqeryExt" />
            var _SlefContext = $(this),
                _CgFormJqeryExt = {

                    On: function (types, selector, data, fn, one) {
                        /// <summary>事件绑定同jQuery的on方法</summary>
                        ...
                        if (data === undefined && fn === undefined && one === undefined) {
                            _fn = _selector;
                            _selector = undefined;
                        }
                        else if (Cmn.IsType(selector, "string") && Cmn.IsType(data, "function")) {
                            _fn = _data;
                            _data = undefined;
                        }
                        ...
                        for (var _key in _types) {
                            ...
                            if (_types[_key] === "click" || _types[_key] === "tap") {
                                ...
                                _fn = function (event) {
                                    ...
                                    if (!_SlefContext["__StartTime__"] ||
                                        _curTime - _SlefContext["__StartTime__"] > 500) {
                                        _origFn.apply(this, arguments);
                                        _SlefContext["__StartTime__"] = new Date().getTime();
                                    }
                                };

                                //如果是 tap 单击事件 并且是在非移动端的话就默认为 click事件
                                //在移动端的话反之
                                if (!Cg.Func.IsMobile()) { _types[_key] = "click"; }
    ...
    (function _GestureEventExd() {
        ...
        var __GestureEventTypes = "tap",
            //原生实现事件类名称
            __OrinEventTypes = Cmn.Func.IsMobile() ? "touchstart touchmove touchend touchcancel"
            ...
            __Utils = {
                ...
                GetDistance: function (pos1, pos2) {
                    /// <summary>获取两点的距离</summary>
                    /// <param name="pos1" type="JSON">起始点</param>
                    /// <param name="pos2" type="JSON">结束点</param>
                    var _x = pos2.x - pos1.x,
                        _y = pos2.y - pos1.y;
                    return Math.sqrt((_x * _x) + (_y * _y));
                },
                //获取屏幕上手指的个数
                GetFingers : function(ev) {
                    /// <summary>获取屏幕上手指的个数</summary>
                    /// <param name="ev" type="event">事件对象</param>
                    return ev.touches ? ev.touches.length : 1;
                },
                CalScale : function(pstart, pmove) {
                    if (pstart.length >= 2 && pmove.length >= 2) {
                        var _disStart = this.GetDistance(pstart[1], pstart[0]);
                        var _disEnd = this.GetDistance(pmove[1], pmove[0]);
                        return _disStart / _disEnd;
                    }
                    return 1;
                },
                //获取两个坐标点的角度
                ...
                GetAngle180 : function(p1, p2) {
                    var _agl = Math.atan((p2.y - p1.y) * -1 / (p2.x - p1.x)) * (180 / Math.PI);
                    return (_agl < 0 ? (_agl + 180) : _agl);
                },
                //获取旋转的距离
                GetDirectionFromAngle : function(agl) {
                    ...
                    for (var key in _directions) {
                        if (_directions[key]) return key;
                    }
                    return null;
                },
                //获取元素相对屏幕的xy的点
                ...
                GetPosOfEvent : function(ev) {
                    if (Cmn.Func.IsMobile() && ev.originalEvent.touches) {
                        ...
                        for (var t = 0, len = ev.originalEvent.touches.length; t < len; t++) {
                            src = ev.originalEvent.touches[t];
                            _posi.push({
                                x: src.pageX,
                                y: src.pageY
                            });
            ...
            //单击事件处理函数
            __Tap = function (ev,ele) {
                
                var __now = Date.now();
                var __touchTime = __now - __StartTime;
                var __movePoint = __Utils.Pos.Move.length ? __Utils.Pos.Move : __Utils.Pos.Start;
                var __distance = __Utils.GetDistance(__Utils.Pos.Start[0], __movePoint[0]);

                if (10 < __distance) return;

                if (650 > __touchTime && __Utils.GetFingers(ev) <= 1) {
                    __tapTimer = setTimeout(function () {

                        $(ele).trigger("tap", {
                            fingersCount: __Utils.GetFingers(ev),
                            position: __Utils.GetPosOfEvent(ev)
                        });
        ...
        //实现手势的事件句柄
        var __GestureEventhandle = function (ev) {

            var __eventTarget = ev.target;
            ...
            switch (ev.type) {
                ...
                case 'mousedown':
                    ...
                    //获取开始的位置
                    if (!__Utils.Pos.Start || __Utils.Pos.Start.length < 2) {
                        __Utils.Pos.Start = __Utils.GetPosOfEvent(ev);
                    };
                ...
                case 'touchmove':
                case 'mousemove':
                    if (!__TouchStart || !__Utils.Pos.Start) return;
                    
                    __Utils.Pos.Move = __Utils.GetPosOfEvent(ev);
                ...
                case 'mouseout':
                    ...
                    if (!__TouchStart) { return false; };
                    __Utils.Pos.Move = __Utils.GetPosOfEvent(ev);
                    __TouchStart = false;
                    __Tap.call(this, ev, __eventTarget);

                    break;
            }
        };

        $.each(__GestureEventTypes.split(" "), function (i, eventName) {
            ...
            jQuery.event.special[eventName] = {
                ...
                setup: function (data, namespaces, eventHandle) {
                    var _self = this;
                    $.each(__OrinEventTypes.split(" "), function (i, eventName) {
                        $(_self).on(eventName, __GestureEventhandle);
                    });
                },
                /** 
                 * 卸载事件处理器 - this指向元素 
                ...
                teardown: function (namespaces) { 
                    var _self = this;
                    $.each(__OrinEventTypes.split(" "), function (i, eventName) {
                        $(_self).off(eventName, __GestureEventhandle);
                    });
                }

            };

            //将事件注册给根元素委托管理 再通过冒泡的形式进行事件广播
